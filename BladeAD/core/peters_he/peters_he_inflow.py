...
def solve_for_steady_state_inflow(
    ...
    for i in range(num_nodes):
        ...
        for nsc in range(ns_cos):
            ...
            phi_vec = compute_phi(r, j, radius_vec[i, :, :] / radius[i, :, :], Q)
        ...
        ux = (inflow + mu_z[i]) * omega[i] * radius[i, :, :]
        ...
        dT = 0.5 * B * rho[i, :, :] * (ux**2 + Vt[i, :, :]**2) * chord_profile[i, :, :] * Cx * dr[i, :, :] * F
        ...
        for nsc in range(ns_cos):
            ...
            phi_cos = compute_phi(m, n, radius_vec[i, :, :]/radius[i, :, :], Q)
            L_cos = dT * phi_cos
        ...
        for nss in range(ns_sin):
            ...
            # compute shape function
            phi_sin = compute_phi(m, n, radius_vec[i, :, :]/radius[i, :, :], Q)
            L_sin = dT * phi_sin
        ...
        dL = 0.5 * B * rho[i, 0, 0] * (ux**2 + Vt[i, :, :]**2) * chord_profile[i, :, :] * Cl * dr[i, :, :]
        thrust = integrate_quantity(dT, scheme=integration_scheme)
